...
if ($id_perusahaan) {
  ...
  $nominal = $_POST['nominal'] ?? die(erid('nominal'));
  if (!$nominal) die('nominal tidak boleh nol atau kosong.');

  $total_pasien = $_POST['total_pasien'] ?? die(erid('total_pasien'));
  if (!$total_pasien) die('total_pasien tidak boleh nol atau kosong.');

  $nama_pasien = $_POST['nama'];
  $nominal_show = number_format($nominal);
  $terbilang = terbilang($nominal);
  $mcu_dan = '';
  $nomor = "$total_pasien pasien";

  $untuk_pembayaran = $_POST['untuk_pembayaran'];
  $Nomor = 'Jumlah Pasien';
  $tanggal_show = $_POST['tanggal_periksa'];
  ...
  $nomor .= "-$id_pasien";
  $untuk_pembayaran = "Biaya $mcu_dan Pemeriksaan Laboratorium";
  $Nomor = 'Nomor';
}
