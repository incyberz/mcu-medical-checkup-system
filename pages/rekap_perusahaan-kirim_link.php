    ...
    foreach ($d as $key => $value) {
      ...
      if ($key == 'kirim_link') {
        ...
        if (!($d['whatsapp'] and $d['nama_kontak'] and $d['nama_kontak'] and $d['gender_kontak']) || $edit_kontak) {
          ...
          $text_wa = "Selamat $waktu $Tn. $d[nama_kontak],%0a%0aBerikut adalah Rekapitulasi MCU Tgl. $last_awal_periksa Karyawan $d[nama_perusahaan]. %0a%0aLink Akses:%0a$link_akses$zid_perusahaan%0a%0a_Mutiara Medical System, $now _";
          $link_wa = "<a target=_blank href='https://api.whatsapp.com/send?phone=$d[whatsapp]&text=$text_wa'>$img_wa</a>";

          $tanggals = '<div>Tanggal Pemeriksaan</div>';
          foreach ($arr_tanggal as $k => $v) {
            $hari = hari_tanggal($v, 1, 1, 0, 0);
            $tanggals .= "
              <div>
                <label>
                  <input type=checkbox class=> $hari
                </label>
              </div>
            ";
          }
          ...
          $value = "
            $tanggals
            <div class='wadah mt2 hideita' id=kirim_ke>Kirim ke : $d[whatsapp], $Tn. $d[nama_kontak] ($d[jabatan_kontak]) ~ $link_wa</div> 
